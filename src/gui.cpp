...
TonemapperScreen::TonemapperScreen() : nanogui::Screen(Eigen::Vector2i(800, 600), "Tone Mapper", true, false) {
    ...
    m_tonemapOperators.push_back(new AcesNarkowiczOperator());
    m_tonemapOperators.push_back(new AcesUnrealOperator());
    m_tonemapOperators.push_back(new AmdOperator());
    ...
    auto *openButton = new Button(m_window, "Open .exr image");
    ...
    m_saveButton = new Button(m_window, "Save .png image");
...
void TonemapperScreen::setImage(const std::string &filename) {
    ...
    m_imageOffset = Eigen::Vector2i(0, 0);
    m_imageScale  = 1.f;
...
bool TonemapperScreen::mouseMotionEvent(const Eigen::Vector2i &p, const Eigen::Vector2i &rel, int button, int modifiers) {
